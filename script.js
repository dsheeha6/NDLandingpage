...
// Initialize Supabase client - using the same approach as the working simple test
let supabase;
...
async function handleFormSubmission(event) {
    ...
    try {
        ...
        // Submit form data to server which will automatically send confirmation email
        console.log('Submitting form data to server for automatic email...');
        const formResponse = await fetch(`${serverUrl}/api/submit-form`, {
            method: 'POST',
            headers: {
        ...
        if (!formResponse.ok) {
            ...
            throw new Error(`Form submission error: ${errorData.error || formResponse.statusText}`);
        }
        
        const formResult = await formResponse.json();
        console.log('Form submission and email result:', formResult);
